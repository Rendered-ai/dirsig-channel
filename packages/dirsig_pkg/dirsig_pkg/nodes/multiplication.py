...
class Multiplication(Node):
    """A simple node that multiplies two numbers together.
    ...
    def exec(self):
        ...
        # Get inputs
        input_a = self.inputs.get('Input A', [1])[0]
        input_b = self.inputs.get('Input B', [1])[0]
        ...
        try:
            ...
            raise ValueError(f"Multiplication node requires numeric inputs. Got A={input_a}, B={input_b}")
        
        # Perform multiplication
        result = num_a * num_b
        
        logger.info(f"Multiplication: {num_a} * {num_b} = {result}")
